...
### 💡 Background & Motivation (개발 배경)
본 프로젝트는 **ACRO 총무부장으로 일하며 매주 직접 겪었던 비효율과 불편함을 자동화 솔루션으로 해결하기 위해 시작**되었습니다. 세션마다 모든 부원들의 출석을 눈으로 확인하고, 지각 시간을 분 단위로 계산하며, 이를 다시 엑셀에 수동으로 입력해 단톡방에 보고하는 일련의 과정은 총무부장을 포함한 운영진들의 소중한 리소스를 낭비시켰습니다. 

사람이 손으로 적다 보니 휴먼 에러나 누락의 리스크가 상존했고, 출결 데이터가 즉시 자산화되지 않아 투명성을 입증하기 번거로웠습니다. 'ACRO Check-in'은 이러한 문제를 백엔드 및 프로세스 자동화를 통해 완벽하게 해결합니다.
---
...
- **배포 자동화 :** GitHub 레포지토리 push와 동시에 Railway 클라우드로 자동 빌드 및 무중단 배포가 수행됩니다.
- **연산 자동화 :** Flask 경량 웹서버 위에서 가동되는 Python 엔진이 지각 및 벌금을 실시간으로 오차 없이 가공합니다.
- **관리 자동화 :** 운영진에게 가장 익숙한 UI인 Google Sheets API를 연동하여 별도의 데이터베이스 관리 도구 학습 없이 출결 현황을 바로 자산화합니다.
